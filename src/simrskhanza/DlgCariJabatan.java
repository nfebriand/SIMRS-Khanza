...
public final class DlgCariJabatan extends javax.swing.JDialog {
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        if (Valid.umurcacheSmc("./cache/jabatan.iyem", 30)) {
            tampilSmc();
        } else {
            tampil2Smc();
        }
    ...
    */
        ...
        if (!ceksukses) {
            ...
            new SwingWorker<Void, String[]>() {
                ...
                protected Void doInBackground() throws Exception {
                    File file = new File("./cache/jabatan.iyem");
                    file.createNewFile();
                    try (FileWriter fw = new FileWriter(file); ResultSet rs = koneksi.createStatement().executeQuery("select jabatan.kd_jbtn, jabatan.nm_jbtn from jabatan")) {
                        ArrayNode array = mapper.createArrayNode();
                        while (rs.next()) {
                            ObjectNode node = mapper.createObjectNode();
                            node.put("KodeJabatan", rs.getString(1));
                            node.put("NamaJabatan", rs.getString(2));
                            array.add(node);
                            if (cari.isBlank()) {
                        ...
                        fw.write(mapper.writeValueAsString(mapper.createObjectNode().set("jabatan", array)));
                        fw.flush();
                    }
                    return null;
                }
                ...
                protected void done() {
                    ...
                    LCount.setText(tabMode.getRowCount() + "");
                    DlgCariJabatan.this.setCursor(Cursor.getDefaultCursor());
                    ceksukses = false;
                }
            }.execute();
        }
    ...
    private void tampil2Smc() {
        if (new File("./cache/jabatan.iyem").isFile() && !Valid.umurcacheSmc("./cache/jabatan.iyem", 7)) {
            if (!ceksukses) {
                ceksukses = true;
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Valid.tabelKosongSmc(tabMode);
                ...
                new SwingWorker<Void, String[]>() {
                    ...
                    @Override
                    protected Void doInBackground() throws Exception {
                        try (FileReader fr = new FileReader("./cache/jabatan.iyem")) {
                            ArrayNode array = mapper.readTree(fr).withArray("jabatan");
                            if (cari.isBlank()) {
                                for (JsonNode node : array) {
                                    publish(new String[] {node.path("KodeJabatan").asText(), node.path("NamaJabatan").asText()});
                                }
                            } else {
                                for (JsonNode node : array) {
                                    if (node.path("KodeJabatan").asText().toLowerCase().contains(cari) || node.path("NamaJabatan").asText().toLowerCase().contains(cari)) {
                                        publish(new String[] {node.path("KodeJabatan").asText(), node.path("NamaJabatan").asText()});
                                    }
                                }
                            }
