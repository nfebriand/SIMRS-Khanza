...
public class P2KMKompilasiBerkasKlaim extends javax.swing.JDialog {
    ...
    private final String KOMPILASIBERKASGUNAKANRIWAYATPASIEN = koneksiDB.KOMPILASIBERKASGUNAKANRIWAYATPASIEN(),
        KODEPJP2KM = "P2K",
        NAMAPJP2KM = Sequel.cariIsiSmc("select penjab.png_jawab from penjab where penjab.kd_pj = ?", KODEPJP2KM),
        KODEPPKP2KM = Sequel.cariIsiSmc("select setting.kode_ppkkemenkes from setting limit 1") + "%";
    ...
    public P2KMKompilasiBerkasKlaim(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        btnInvoice.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        tabMode = new DefaultTableModel(null, new Object[] {
            "P", "No. Rawat", "NIK", "No. RM", "Nama Pasien", "Status Rawat",
            "Tgl. Masuk", "Tgl. Pulang", "Status Pulang", "Unit/Poli", "DPJP",
            ...
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                ...
                if (column == 2 && Boolean.TRUE.equals(table.getValueAt(row, 13))) {
                    if (component instanceof javax.swing.JLabel) {
                        ...
                        lbl.setToolTipText("Data ini memiliki catatan progress berkas");
                    }
    ...
    private void initComponents() {
        ...
        TanggalPulang.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "07-05-2026 12:17:21" }));
        ...
        TanggalKematian.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "07-05-2026" }));
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Kompilasi Berkas Klaim P2KM ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        jLabel26.setText("Resume Ralan :");
        jLabel26.setName("jLabel26"); // NOI18N
        jLabel26.setPreferredSize(new java.awt.Dimension(105, 14));
        panelBiasa1.add(jLabel26);
        jLabel26.setBounds(200, 150, 90, 16);
        ...
        btnResumeRalan.setBounds(290, 150, 90, 16);
        ...
        internalFrame1.getAccessibleContext().setAccessibleName("::[ Kompilasi Berkas Klaim P2KM ]::");
    ...
    private void BtnAllActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnAllActionPerformed
        kodePJ.setText(KODEPJP2KM);
        namaPJ.setText(NAMAPJP2KM);
        ...
        if (!isLoading) {
            ...
            new SwingWorker<Void, Object[]>() {
                ...
                protected Void doInBackground() throws Exception {
                    ...
                    String statusrawat = "";
                    if (idstatusrawat == 1) {
                        statusrawat = "and r.status_lanjut = 'Ralan' ";
                    } else if (idstatusrawat == 2) {
                        statusrawat = "and r.status_lanjut = 'Ranap' ";
                    }

                    // Anchor: reg_periksa. NIK (px.no_ktp) dipakai sebagai no_sep ke tabel klaim.
                    // Struktur tabel klaim tidak diubah — aplikasi grouper tetap mengisi no_sep dengan NIK.
                    try (PreparedStatement ps = koneksi.prepareStatement(
                        "select r.no_rawat, px.no_ktp as no_sep, r.no_rkm_medis, px.nm_pasien, r.status_lanjut, " +
                        "date(r.tgl_registrasi) as tglsep, " +
                        "case when r.status_lanjut = 'Ranap' then date(ki.tgl_keluar) else date(r.tgl_registrasi) end as tglpulang, " +
                        "ki.stts_pulang, " +
                        "case when r.status_lanjut = 'Ranap' then concat(ki.kd_kamar, ' ', b.nm_bangsal) " +
                        "     when r.status_lanjut = 'Ralan' then p.nm_poli end as ruangan, " +
                        "d.nm_dokter, " +
                        "case when inc.no_sep is not null then 1 " +
                        "when idg.no_sep is not null and idf.no_sep is not null and ing.no_sep is not null and inf.no_sep is not null and inc.no_sep is null then 2 " +
                        "when idg.no_sep is not null and idf.no_sep is not null and ing.no_sep is not null and (left(ing.code_cbg, 1) != 'X') and inf.no_sep is null and inc.no_sep is null then 3 " +
                        "when idg.no_sep is not null and idf.no_sep is not null and (ing.no_sep is null or (ing.no_sep is not null and (left(ing.code_cbg, 1) = 'X'))) and inf.no_sep is null and inc.no_sep is null then 4 " +
                        "when idg.no_sep is not null and idg.mdc_number != '36' and idf.no_sep is null and inf.no_sep is null and inc.no_sep is null then 5 " +
                        "when (idg.no_sep is null or (idg.no_sep is not null and idg.mdc_number = '36')) and idf.no_sep is null and inf.no_sep is null and inc.no_sep is null then 6 " +
                        "end as statusklaim, " +
                        "if(cbk.no_sep is not null, 1, 0) as ada_catatan " +
                        "from reg_periksa r " +
                        "join pasien px on r.no_rkm_medis = px.no_rkm_medis " +
                        "join poliklinik p on r.kd_poli = p.kd_poli " +
                        "left join kamar_inap ki on r.no_rawat = ki.no_rawat and ki.stts_pulang != 'Pindah Kamar' " +
                        "left join kamar k on ki.kd_kamar = k.kd_kamar " +
                        "left join bangsal b on k.kd_bangsal = b.kd_bangsal " +
                        "left join dokter d on r.kd_dokter = d.kd_dokter " +
                        "left join idrg_grouping_smc idg on px.no_ktp = idg.no_sep " +
                        "left join idrg_klaim_final_smc idf on px.no_ktp = idf.no_sep " +
                        "left join inacbg_grouping_stage12 ing on px.no_ktp = ing.no_sep " +
                        "left join inacbg_klaim_final_smc inf on px.no_ktp = inf.no_sep " +
                        "left join inacbg_cetak_klaim inc on px.no_ktp = inc.no_sep " +
                        "left join catatan_berkas_klaim cbk on px.no_ktp = cbk.no_sep " +
                        "where r.tgl_registrasi between ? and ? " +
                        "and r.status_bayar = 'Sudah Bayar' " +
                        "and px.no_ktp is not null and px.no_ktp != '' " +
                        "and r.kd_pj = ? " +
                        statusrawat + statusklaim +
                        (cari.isBlank() ? "" :
                            "and (px.no_ktp like ? or r.no_rawat like ? or r.no_rkm_medis like ? " +
                            "or px.nm_pasien like ? or p.nm_poli like ? " +
                            "or concat(ki.kd_kamar, ' ', b.nm_bangsal) like ? or d.nm_dokter like ?) ") +
                        "group by r.no_rawat, r.no_rkm_medis, px.no_ktp " +
                        "order by r.tgl_registrasi desc, r.status_lanjut desc"
                    )) {
                        int p = 0;
                        ps.setString(++p, tglawal);
                        ps.setString(++p, tglakhir);
                        // Selalu filter kd_pj = KODEPJP2KM (default), atau kode penjamin yang dipilih user
                        ps.setString(++p, kodePJ.getText().isBlank() ? KODEPJP2KM : kodePJ.getText());
                        if (!cari.isBlank()) {
                            ps.setString(++p, "%" + cari + "%");
                            ps.setString(++p, "%" + cari + "%");
                            ps.setString(++p, "%" + cari + "%");
                ...
                protected void done() {
                    ...
                    P2KMKompilasiBerkasKlaim.this.setCursor(Cursor.getDefaultCursor());
                    isLoading = false;
                }
            }.execute();
        }
    ...
    private void getData() {
        if (selectedRow >= 0) {
            ...
            // Cek keberadaan berkas langsung via no_rawat — tidak butuh bridging_sep
            // ada_skdp dan ada_spri tidak ada di P2KM (dokumen eksklusif BPJS), di-hardcode false
            try (PreparedStatement ps = koneksi.prepareStatement(
                "select " +
                "exists(select 1 from data_triase_igd t       where t.no_rawat = ?) as ada_triase, " +
                "exists(select 1 from resume_pasien_ranap rr  where rr.no_rawat = ?) as ada_resume_ranap, " +
                "exists(select 1 from resume_pasien rp        where rp.no_rawat = ?) as ada_resume_ralan, " +
                "exists(select 1 from penilaian_medis_igd pm  where pm.no_rawat = ?) as ada_awal_medis_igd, " +
                "exists(select 1 from periksa_lab pl          where pl.no_rawat = ?) as ada_periksa_lab, " +
                "exists(select 1 from periksa_radiologi pr    where pr.no_rawat = ?) as ada_periksa_rad, " +
                "exists(select 1 from billing bl              where bl.no_rawat = ?) as ada_billing"
            )) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        // SPRI dan Surkon: P2KM tidak punya (dokumen eksklusif BPJS)
    ...
    private void setFlagKlaim() {
        try (PreparedStatement ps = koneksi.prepareStatement(
            // Anchor: pasien. Join ke tabel klaim via NIK (no_ktp = no_sep di tabel klaim).
            // bridging_sep tidak dipakai — pasien P2KM tidak ada di sana.
    ...
    private void exportResumeRanap(final String urutan, final boolean ada, final int row) throws KompilasiException {
        ...
        try {
            ...
            simpanPDF(tbKompilasi.getValueAt(row, 2).toString(), "rptLaporanResumeRanapKompilasi.jasper", urutan + "_ResumePasienRanap", param);
        } catch (Exception e) {
            throw new KompilasiException("Resume Ranap Pasien", urutan, tbKompilasi.getValueAt(row, 2).toString(), e);
        }
    }

    private void exportResumeRalan(final String urutan, final boolean ada, final int row)throws KompilasiException {
        if (!ada) {
            return;
        }
        ...
        try {        
            ...
            param.put("norawat",tbKompilasi.getValueAt(row, 1).toString());
            tanggal="";
            if(Sequel.cariIsi("select reg_periksa.status_lanjut from reg_periksa where reg_periksa.no_rawat=?",tbKompilasi.getValueAt(row, 1).toString()).equals("Ralan")){
                param.put("ruang",Sequel.cariIsi("select poliklinik.nm_poli from poliklinik inner join reg_periksa on reg_periksa.kd_poli=poliklinik.kd_poli where reg_periksa.no_rawat=?",tbKompilasi.getValueAt(row, 1).toString()));
                tanggal=Sequel.cariIsi("select DATE_FORMAT(tgl_registrasi, '%d-%m-%Y') from reg_periksa where no_rawat=?",tbKompilasi.getValueAt(row, 1).toString());
                param.put("tanggalkeluar",tanggal);
            }else{
                param.put("ruang",Sequel.cariIsi("select nm_bangsal from bangsal inner join kamar inner join kamar_inap on bangsal.kd_bangsal=kamar.kd_bangsal and kamar_inap.kd_kamar=kamar.kd_kamar where no_rawat=? order by tgl_masuk desc limit 1 ",tbKompilasi.getValueAt(row, 1).toString()));
                tanggal=Sequel.cariIsi("select DATE_FORMAT(tgl_keluar, '%d-%m-%Y') from kamar_inap where no_rawat=? order by tgl_keluar desc limit 1 ",tbKompilasi.getValueAt(row, 1).toString());
                param.put("tanggalkeluar",tanggal);
            }
            finger=Sequel.cariIsi("select sha1(sidikjari.sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?",kodeDokter);
            param.put("finger","Dikeluarkan di "+akses.getnamars()+", Kabupaten/Kota "+akses.getkabupatenrs()+"\nDitandatangani secara elektronik oleh "+namaDokter+"\nID "+(finger.equals("")? kodeDokter:finger)+"\n"+tanggal); 
    ...
    private void exportRiwayatPasienRanap(final String urutan, final boolean ada, final int row) throws Exception {
        if (resume == null) {
            resume = new RMRiwayatPerawatan(null, false);
        }
        try {
            resume.kompilasiDariRiwayatRanap(tbKompilasi.getValueAt(row, 1).toString(), tbKompilasi.getValueAt(row, 3).toString(), tanggalExport, tbKompilasi.getValueAt(row, 2).toString(), urutan);
        } catch (Exception e) {
            throw new KompilasiException("Riwayat Pasien", urutan, tbKompilasi.getValueAt(row, 2).toString(), e);
        }
    }
            ...
            protected Void doInBackground() throws Exception {
                ...
                for (int i = 0, j = 0; i < tbKompilasi.getRowCount(); i++) {
                    ...
                    if ((Boolean) tbKompilasi.getValueAt(i, 0)) {
                        ...
                        // P2KM: tidak lookup bridging_sep — cek berkas langsung via no_rawat
                        String noRawatBulk = tbKompilasi.getValueAt(i, 1).toString();
                        String nikBulk     = tbKompilasi.getValueAt(i, 2).toString(); // kolom 2 = NIK

                        try (PreparedStatement ps = koneksi.prepareStatement(
                            "select " +
                            "exists(select 1 from data_triase_igd t       where t.no_rawat = ?) as ada_triase_igd, " +
                            "exists(select 1 from pemeriksaan_ralan pr    where pr.no_rawat = ?) as ada_soap, " +
                            "exists(select 1 from resume_pasien_ranap r   where r.no_rawat = ?) as ada_resume_ranap, " +
                            "exists(select 1 from resume_pasien rp        where rp.no_rawat = ?) as ada_resume_ralan, " +
                            "exists(select 1 from penilaian_medis_igd p   where p.no_rawat = ?) as ada_awal_medis_igd, " +
                            "exists(select 1 from periksa_lab pl          where pl.no_rawat = ?) as ada_periksa_lab, " +
                            "exists(select 1 from periksa_radiologi pr    where pr.no_rawat = ?) as ada_periksa_rad, " +
                            "exists(select 1 from inacbg_cetak_klaim i    where i.no_sep    = ?) as ada_hasil_klaim, " +
                            "exists(select 1 from berkas_digital_perawatan b where b.no_rawat = ?) as ada_berkas_digital"
                        )) {
                            ps.setString(1, noRawatBulk);
                            ps.setString(2, noRawatBulk);
                            ps.setString(3, noRawatBulk);
                            ps.setString(4, noRawatBulk);
                            ps.setString(5, noRawatBulk);
                            ps.setString(6, noRawatBulk);
                            ps.setString(7, noRawatBulk);
                            ps.setString(8, nikBulk);   // ada_hasil_klaim via NIK
                            ps.setString(9, noRawatBulk);
                            try (ResultSet rs = ps.executeQuery()) {
                                while (rs.next()) {
                                    if (gunakanTanggalExport.equals("sep")) {
