    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("DataSkriningInstrumenAMT.html", "DATA SKRINING INSTRUMEN AMT", tbObat);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("DataSkriningInstrumenAMT.wps", "DATA SKRINING INSTRUMEN AMT", tbObat);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("DataSkriningInstrumenAMT.csv", tbObat);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("DataSkriningInstrumenAMT.xlsx", tbObat);
                        break;
                }
            } catch (Exception e) {
