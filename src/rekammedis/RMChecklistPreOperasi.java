    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("DataChecklistPreOperasi.html", "DATA CHECK LIST PRE OPERASI", tbObat);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("DataChecklistPreOperasi.wps", "DATA CHECK LIST PRE OPERASI", tbObat);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("DataChecklistPreOperasi.csv", tbObat);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("DataChecklistPreOperasi.xlsx", tbObat);
                        break;
                }
            } catch (Exception e) {
