...
public class DlgRL4ASebab extends javax.swing.JDialog {
    ...
    // Variabel Baru Sesuai Kebutuhan (Tanpa Jam karena keterbatasan data)
    ...
    */
    ...
    private void prosesCari() {
        ...
        try {
            ...
            // Membuat Header Tabel HTML Baru
            htmlContent.append(
                "<tr class='isi'>"+
                    ...
                    "<td valign='middle' bgcolor='#FFFAFA' align='center' width='3%' rowspan='3'>Jumlah Pasien Keluar Hidup (Σ Kolom L + Σ Kolom P)</td>"+
            ...
            // Baris nomor kolom (Contoh, sesuaikan nomornya jika perlu)
            ...
            // Query Utama (Berdasarkan Tanggal Masuk)
            ...
            try {
                ...
                while(rs.next()){
                    // Reset semua counter untuk setiap diagnosa baru
                    ...
                    // Query Pasien per Diagnosa (Berdasarkan Tanggal Masuk)
                    ...
                    try {
                        ...
                        while(rs2.next()){
                            // Logika Pengelompokan Usia BARU
                            ...
                            if (sttsumur.equals("Hr")) {
                                ...
                                } else if (umurdaftar >= 29) { // Dianggap masuk ke kelompok 29hr-<3bln
                                    if (jk.equals("L")) hr29k3blnl++; else hr29k3blnp++;
                                }
                                // Tidak ada penanganan untuk < 1 hari (atau jam)
                            } else if (sttsumur.equals("Bl")) {
                                if (umurdaftar < 3) { // Bulan 0, 1, 2 dianggap masuk kelompok 29hr-<3bln
                                    if (jk.equals("L")) hr29k3blnl++; else hr29k3blnp++;
                                } else if (umurdaftar >= 3 && umurdaftar <= 5) { // 3-5 bulan
                                    if (jk.equals("L")) bln3s5l++; else bln3s5p++;
                                } else if (umurdaftar >= 6 && umurdaftar <= 11) { // 6-11 bulan
                    ...
                    // Query Pasien Mati (Berdasarkan Tanggal Masuk)
                    ps3=koneksi.prepareStatement(
                            ...
                            "and reg_periksa.tgl_registrasi between ? and ? and diagnosa_pasien.kd_penyakit=? "); // group by tidak perlu jika hanya 1 diagnosa
                    try {
                        ...
                        if(rs2.next()){ // Cukup pakai if karena hanya mengambil count
                            mati=rs2.getInt(1);
                        }
                    } catch (Exception e) {
    ...
    private void prosesCari2() {
       this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        try {
            ...
            // Query Utama (BEDA: Berdasarkan Tanggal Keluar & Prioritas 1)
            ps=koneksi.prepareStatement("select diagnosa_pasien.kd_penyakit,SUBSTRING(penyakit.nm_penyakit,1,80) as nm_penyakit from diagnosa_pasien inner join penyakit "+
            ...
            try {
                ...
                while(rs.next()){
                    ...
                    // Query Pasien per Diagnosa (BEDA: Berdasarkan Tanggal Keluar)
                    ps2=koneksi.prepareStatement(
                            "select diagnosa_pasien.kd_penyakit,reg_periksa.umurdaftar,reg_periksa.sttsumur,pasien.jk "+
                            "from diagnosa_pasien inner join reg_periksa inner join pasien inner join kamar_inap "+
                            "on reg_periksa.no_rawat=diagnosa_pasien.no_rawat and reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                            "and kamar_inap.no_rawat=reg_periksa.no_rawat where diagnosa_pasien.status='Ranap' "+ // Mungkin perlu tambah diagnosa_pasien.prioritas='1' jika hanya diagnosa utama keluar yang dihitung per pasien
                    ...
                    // Query Pasien Mati (BEDA: Berdasarkan Tanggal Keluar)
                    ps3=koneksi.prepareStatement(
                        "select count(pasien_mati.no_rkm_medis) " +
                        ...
                        "inner join kamar_inap on kamar_inap.no_rawat = reg_periksa.no_rawat " + // Join kamar_inap
                        "where diagnosa_pasien.status = 'Ranap' " + // Mungkin perlu tambah diagnosa_pasien.prioritas='1'
                        "and kamar_inap.tgl_keluar between ? and ? " + // Filter tanggal keluar
                        "and diagnosa_pasien.kd_penyakit = ? "); // group by tidak perlu
                    try {
                        ps3.setString(1,Valid.SetTgl(Tgl1.getSelectedItem()+""));
                        ps3.setString(2,Valid.SetTgl(Tgl2.getSelectedItem()+""));
                        ps3.setString(3,rs.getString("kd_penyakit"));
