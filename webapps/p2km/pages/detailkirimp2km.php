                    ...
                    <table width="100%" align="center">
                        <?php if ($grouper === 'idrg'): ?>
                            ...
                            <?php else: ?>
                                ...
                                <?php $hasilgroupingidrg = mysqli_fetch_assoc(bukaquery("select * from idrg_grouping_p2km where no_sep = '$nosep'")); ?>
                                ...
                                <?php
                                    $diagnosa_idrg = '';
                                    $querydiagnosa_idrg = bukaquery("select i.kode_icd10, r.deskripsi, i.urut from idrg_diagnosa_pasien_p2km i join idrg_referensi_icd10_smc r on i.kode_icd10 = r.code1 where i.no_sep = '$nosep' order by i.urut asc");
                                ?>
                                ...
                                <?php
                                    $prosedur_idrg = '';
                                    $queryprosedur_idrg = bukaquery("select i.kode_icd9, r.deskripsi, i.multiplicity, i.urut from idrg_prosedur_pasien_p2km i join idrg_referensi_icd9cm_smc r on i.kode_icd9 = r.code1 where i.no_sep = '$nosep' order by i.urut asc");
                                ?>
                                ...
                                <?php
                                    ...
                                    // Coba ambil data via bridging_sep (alur BPJS normal)
                                    $baris = mysqli_fetch_array(bukaquery(<<<SQL
                                        ...
                                        pasien.nm_pasien, pasien.jk, pasien.umur, pasien.tgl_lahir, pasien.no_ktp, (select inacbg_pasien_tb_p2km.no_sitb from inacbg_pasien_tb_p2km where
                                        inacbg_pasien_tb_p2km.no_rkm_medis = reg_periksa.no_rkm_medis limit 1) as no_sitb, dokter.nm_dokter, poliklinik.nm_poli, penjab.png_jawab,
                                    ...
                                    // Fallback P2KM: nosep berisi NIK, cari via pasien.no_ktp
                                    if (!$baris) {
                                        $baris = mysqli_fetch_array(bukaquery(<<<SQL
                                            select pasien.no_ktp as no_sep, reg_periksa.tgl_registrasi as tglsep, '' as asal_rujukan, '' as no_kartu,
                                            case when reg_periksa.status_lanjut = 'Ranap'
                                                 then (select date(ki2.tgl_keluar) from kamar_inap ki2 where ki2.no_rawat = reg_periksa.no_rawat and ki2.stts_pulang != 'Pindah Kamar' order by ki2.tgl_keluar desc limit 1)
                                                 else date(reg_periksa.tgl_registrasi)
                                            end as tglpulang,
                                            pasien.nm_pasien, pasien.jk, pasien.umur, pasien.tgl_lahir, pasien.no_ktp,
                                            (select inacbg_pasien_tb_p2km.no_sitb from inacbg_pasien_tb_p2km where inacbg_pasien_tb_p2km.no_rkm_medis = reg_periksa.no_rkm_medis limit 1) as no_sitb,
                                            dokter.nm_dokter, poliklinik.nm_poli, penjab.png_jawab,
                                            reg_periksa.*, if(reg_periksa.status_lanjut = 'Ranap', '1', '2') as jnsrawat
                                            from reg_periksa
                                    ...
                                    $saved_klaim    = mysqli_fetch_assoc(bukaquery("select * from inacbg_data_klaim_p2km where no_sep = '$nosep' limit 1"));
                                    $sistole        = '120';
                                    $diastole       = '90';
                                    ...
                                    // P2KM: bridging_sep tidak ada, naikkelas tetap kosong — sudah ditangani oleh empty() di atas
                                    ...
                                    // P2KM: bridging_sep tidak ada, hemodialisa dicek via poliklinik langsung
                                    $dializer_single_use = getOne("select exists(select * from bridging_sep where no_sep = '$nosep' and nmpolitujuan like 'hemodial%')");
                                    if ($dializer_single_use === '0' || $dializer_single_use === null) {
                                        // fallback P2KM: cek via nama poli di reg_periksa
                                        $dializer_single_use = getOne("select exists(select * from reg_periksa join poliklinik on reg_periksa.kd_poli = poliklinik.kd_poli where reg_periksa.no_rawat = '$norawat' and poliklinik.nm_poli like 'hemodial%')");
                                    }
                                    ...
                                    // Kelas rawat — fallback P2KM: ambil dari kamar jika tidak ada di bridging_sep
                                    $kelas = ($status_lanjut !== 'Ralan') ? getOne("select klsrawat from bridging_sep where no_sep = '$nosep'") : '';
                                    if (empty($kelas) && $status_lanjut !== 'Ralan') {
                                        $kelas = getOne("select kamar.kelas from kamar join kamar_inap on kamar.kd_kamar = kamar_inap.kd_kamar where kamar_inap.no_rawat = '$norawat' order by kamar_inap.tgl_masuk desc limit 1");
                                    }
                                    ...
                                    $delivery       = [];
                                    $kelahiran_rows = [];
                                    $querykelahiran = bukaquery("select * from inacbg_data_klaim_persalinan_p2km where no_sep = '$nosep' order by delivery_sequence asc");
                        ...
                        <?php elseif ($grouper === 'inacbg_stage1'): ?>
                            ...
                            <?php $querydiagnosa_idrg = bukaquery("select i.kode_icd10, r.deskripsi, i.urut from idrg_diagnosa_pasien_p2km i join idrg_referensi_icd10_smc r on i.kode_icd10 = r.code1 where i.no_sep = '$nosep' order by i.urut asc"); ?>
                            <?php while ($barisdiagnosa_idrg = mysqli_fetch_array($querydiagnosa_idrg)): ?>
                                <?php if ($barisdiagnosa_idrg['urut'] == '1'): ?>
                                    <tr class="head">
                                        <td width="28%">Diagnosa</td>
                                ...
                                <?php endif; ?>
                            <?php endwhile; ?>
                            <?php $queryprosedur_idrg = bukaquery("select i.kode_icd9, r.deskripsi, i.multiplicity, i.urut from idrg_prosedur_pasien_p2km i join idrg_referensi_icd9cm_smc r on i.kode_icd9 = r.code1 where i.no_sep = '$nosep' order by i.urut asc"); ?>
                            <?php while ($barisprosedur_idrg = mysqli_fetch_array($queryprosedur_idrg)): ?>
                                <?php if ($barisprosedur_idrg['urut'] == '1'): ?>
                                    <tr class="head">
                                        <td width="28%">Prosedur</td>
                            ...
                            <?php
                                $diagnosa_inacbg = '';
                                $querydiagnosa_inacbg = bukaquery("select kode_icd10, deskripsi, urut, keterangan from inacbg_diagnosa_pasien_p2km where no_sep = '$nosep' order by urut asc");
                            ?>
                            ...
                            <?php
                                $prosedur_inacbg = '';
                                $queryprosedur_inacbg = bukaquery("select kode_icd9, deskripsi, urut, keterangan from inacbg_prosedur_pasien_p2km where no_sep = '$nosep' order by urut asc");
                            ?>
                        ...
                        <?php elseif ($grouper === 'inacbg_final'): ?>
                            ...
                            <?php $querydiagnosa_inacbg = bukaquery("select kode_icd10, deskripsi, urut, keterangan from inacbg_diagnosa_pasien_p2km where no_sep = '$nosep' order by urut asc"); ?>
                            <?php while ($barisdiagnosa_inacbg = mysqli_fetch_array($querydiagnosa_inacbg)): ?>
                                <?php if ($barisdiagnosa_inacbg['urut'] == '1'): ?>
                                    <tr class="head">
                                        <td width="28%">Diagnosa INACBG</td>
                                ...
                                <?php endif; ?>
                            <?php endwhile; ?>
                            <?php $queryprosedur_inacbg = bukaquery("select kode_icd9, deskripsi, urut, keterangan from inacbg_prosedur_pasien_p2km where no_sep = '$nosep' order by urut asc"); ?>
                            <?php while ($barisprosedur_inacbg = mysqli_fetch_array($queryprosedur_inacbg)): ?>
                                <?php if ($barisprosedur_inacbg['urut'] == '1'): ?>
                                    <tr class="head">
                                        <td width="28%">Prosedur INACBG</td>
                            ...
                            <?php
                                ...
                                $querycmg = bukaquery2("select cmg_code, cmg_description, cmg_type from inacbg_grouping_stage2_p2km where no_sep = '$nosep'");
                        ...
                        <?php elseif ($grouper === 'final'): ?>
                            <?php
                                if ($action === 'reedit') {
                                    ...
                                    // Fallback P2KM: nosep adalah NIK, cari no_rawat via pasien.no_ktp
                                    if (empty($norawat_edit)) {
                                        $norawat_edit = getOne("select reg_periksa.no_rawat from reg_periksa join pasien on reg_periksa.no_rkm_medis = pasien.no_rkm_medis where pasien.no_ktp = '$nosep' order by reg_periksa.tgl_registrasi desc limit 1");
                                    }
