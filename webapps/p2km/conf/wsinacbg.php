<?php
    ...
    function getKey()
    {
       $keyRS = "4face4bbd35ae4f768e2e4c71299ca3a09c88f865b9fd0ee7466cfbff871226a";
    ...
    function SimpanGetDataKlaimSmc($nomor_sep, $data)
    {
        ...
        bukaquery2("delete from inacbg_data_klaim_p2km where no_sep = '$nomor_sep'");
        InsertData2('inacbg_data_klaim_p2km', sprintf(
        ...
        foreach (($data['tarif_rs'] ?? []) as $tarif => $nilai) {
            InsertData2('inacbg_data_klaim_tarif_p2km', sprintf("'%s', '%s', '%s'", $nomor_sep, $tarif, $nilai));
        }
    }
    ...
    function ReeditKlaimSmc($nomor_sep, $norawat)
    {
        ...
        if ($msg['metadata']['code'] != '200') {
            ...
            @bukaquery2("delete from idrg_grouping_p2km where no_sep = '$nomor_sep'");
            @bukaquery2("delete from idrg_klaim_final_p2km where no_sep = '$nomor_sep'");
            @bukaquery2("delete from inacbg_diagnosa_pasien_p2km where no_sep = '$nomor_sep'");
            @bukaquery2("delete from inacbg_prosedur_pasien_p2km where no_sep = '$nomor_sep'");
            @bukaquery2("delete from inacbg_grouping_stage12 where no_sep = '$nomor_sep'");
            @bukaquery2("delete from inacbg_grouping_stage2_p2km where no_sep = '$nomor_sep'");
            @bukaquery2("delete from inacbg_klaim_final_p2km where no_sep = '$nomor_sep'");
    ...
    function UpdateDataKlaimSmc(
        ...
        bukaquery2("delete from inacbg_data_klaim_p2km where no_sep = '$nomor_sep'");
        InsertData2('inacbg_data_terkirim2', "'$nomor_sep', '$coder_nik'");
        InsertData2('inacbg_data_klaim_p2km', sprintf("'%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s'",
    ...
    function ValidasiRegistrasiSITBSmc($nomor_sep, $nomor_rm, $nomor_sitb)
    {
        ...
        bukaquery2(sprintf("insert into inacbg_pasien_tb_p2km values ('%s', '%s', '%s', '%s') on duplicate key update no_rkm_medis = values(no_rkm_medis), no_sitb = values(no_sitb), status_validasi = values(status_validasi)",
    ...
    function GroupingStage1IdrgSmc($nomor_sep, $norawat, $status_rawat, $coder_nik)
    {
        ...
        try {
            bukaquery2(sprintf(<<<'SQL'
                insert into idrg_grouping_p2km values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') on duplicate key update
    ...
    function FinalIdrgSmc($nomor_sep, $coder_nik)
    {
        ...
        InsertData2('idrg_klaim_final_p2km', "'$nomor_sep', '$coder_nik'");
    ...
    function ImportIdrgToInacbgSmc($nomor_sep)
    {
        ...
        foreach ($data_diagnosa as $dx) {
            ...
            try {
                bukaquery2(sprintf("insert into inacbg_diagnosa_pasien_p2km values ('%s', '%s', '%s', %s, '%s', %s)", $nomor_sep, $dx['code'], cleankar($dx['display']), $dx['no'], $keterangan, $locked));
            } catch (\Exception $e) {
                continue;
            }
        }
        ...
        foreach ($data_prosedur as $p) {
            ...
            try {
                bukaquery2(sprintf("insert into inacbg_prosedur_pasien_p2km values ('%s', '%s', '%s', %s, '%s', %s)", $nomor_sep, $p['code'], cleankar($p['display']), $p['no'], $keterangan, $locked));
            } catch (\Exception $e) {
                continue;
            }
        }
    ...
    function GroupingStage2InacbgSmc($nomor_sep, $coder_nik, $special_cmg)
    {
        ...
        if (!empty($msg['response_inacbg']['special_cmg'])) {
            ...
            foreach ($msg['response_inacbg']['special_cmg'] as $special_cmg) {
                InsertData2('inacbg_grouping_stage2_p2km', sprintf("'%s', '%s', '%s', '%s', %s",
                    $nomor_sep,
                    $special_cmg['code'],
    ...
    function FinalInacbgSmc($nomor_sep, $coder_nik)
    {
        ...
        InsertData2('inacbg_klaim_final_p2km', "'$nomor_sep', '$coder_nik'");
    ...
    function FinalisasiKlaimSmc($nomor_sep, $coder_nik)
    {
        ...
        // Cari no_rawat via bridging_sep (BPJS normal)
        $norawat = trim(getOne("select bridging_sep.no_rawat from bridging_sep where bridging_sep.no_sep = '$nomor_sep'"));

        // Fallback P2KM: nomor_sep adalah NIK, cari no_rawat via pasien.no_ktp
        if (empty($norawat)) {
            $norawat = trim(getOne("select reg_periksa.no_rawat from reg_periksa join pasien on reg_periksa.no_rkm_medis = pasien.no_rkm_medis where pasien.no_ktp = '$nomor_sep' order by reg_periksa.tgl_registrasi desc limit 1"));
        }

        $status_lanjut = trim(getOne("select reg_periksa.status_lanjut from reg_periksa where reg_periksa.no_rawat = '$norawat'"));

        if (!empty($norawat) && $import_koding === 'IDRG') {
            ...
            // Gunakan no_rawat langsung — tidak join bridging_sep (kompatibel P2KM & BPJS)
            bukaquery2(<<<SQL
                insert into diagnosa_pasien (no_rawat, kd_penyakit, status, prioritas, status_penyakit) select * from (select '$norawat' as no_rawat,
                ...
                idrg_diagnosa_pasien_p2km i join reg_periksa r on r.no_rawat = '$norawat' where
                i.no_sep = '$nomor_sep') t
                SQL);

            bukaquery2(<<<SQL
                insert into prosedur_pasien (no_rawat, kode, status, prioritas, jumlah) select * from (select '$norawat' as no_rawat, i.kode_icd9 as kode,
                r.status_lanjut as status, i.urut as prioritas, i.multiplicity as jumlah from idrg_prosedur_pasien_p2km i join reg_periksa r on r.no_rawat = '$norawat'
                where i.no_sep = '$nomor_sep') t
                SQL);
        } else if (!empty($norawat) && $import_koding === 'INA-CBG') {
            bukaquery2("delete from diagnosa_pasien where diagnosa_pasien.no_rawat = '$norawat' and diagnosa_pasien.status = '$status_lanjut'");
            bukaquery2("delete from prosedur_pasien where prosedur_pasien.no_rawat = '$norawat' and prosedur_pasien.status = '$status_lanjut'");
            ...
            bukaquery2(<<<SQL
                ...
                i.kode_icd10 as kd_penyakit, r.status_lanjut as status, i.urut as prioritas, if(exists(select * from diagnosa_pasien d join
                reg_periksa r2 on d.no_rawat = r2.no_rawat where r2.no_rkm_medis = r.no_rkm_medis), 'Lama', 'Baru') as status_penyakit from
                inacbg_diagnosa_pasien_p2km i join reg_periksa r on r.no_rawat = '$norawat' where
                i.no_sep = '$nomor_sep') t
                SQL);

            bukaquery2(<<<SQL
                insert into prosedur_pasien (no_rawat, kode, status, prioritas, jumlah) select * from (select '$norawat' as no_rawat, i.kode_icd9 as kode,
                r.status_lanjut as status, i.urut as prioritas, 1 as jumlah from inacbg_prosedur_pasien_p2km i join reg_periksa r on r.no_rawat = '$norawat'
                where i.no_sep = '$nomor_sep') t
                SQL);
        }
